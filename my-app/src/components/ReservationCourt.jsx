...
const ReservationCourt = () => {
  ...
  const getData = async () => {
    ...
    const result = await axios.get(
      `https://pf-henry-backend-production.up.railway.app/turn/complejo-turno-date/${newFormatDate}/${court?.id}`
    );
  ...
  const saveTurn = async (time) => {
    ...
    const result = await axios.post(
      "https://pf-henry-backend-production.up.railway.app/turn/create",
      turnCreate
    );
  };
  const payment = async () => {
    const result = await axios
      .post("https://pf-henry-backend-production.up.railway.app/payment", {
        id: "1",
        name,
        image:
